...
OpenAI Chat Completions streaming 也正式辨識 `stream_options`。`include_usage=true` 時，每個一般 SSE chunk 會帶 `usage:null`，在唯一的 `data: [DONE]` 前再送一個 `choices:[]` 的 usage chunk；`include_usage=false` 不增加 usage chunk。ChatHub 目前沒有提供可對應 OpenAI `prompt_tokens` / `completion_tokens` 的 authoritative token accounting，因此這些數值沿用 Sidecar 的可見 request/completion estimator，並在 `m365.usage_source`、`usage_values_are_estimates=true` 與 `usage_estimate_scope=visible_request_and_completion` 明確標示來源，不能當成 Microsoft 計費 token。現行 OpenAI `stream_options.include_obfuscation` 會被辨識但標成 ignored，Sidecar 不會假裝已實作 stream obfuscation。
...
OpenAI Chat Completions streaming also recognizes `stream_options` as a first-class compatibility field. With `include_usage=true`, ordinary SSE chunks carry `usage:null`, followed by exactly one `choices:[]` usage chunk before the single `data: [DONE]`; `include_usage=false` adds no usage chunk. ChatHub currently exposes no authoritative token accounting that maps to OpenAI `prompt_tokens` / `completion_tokens`, so these values reuse the Sidecar's visible request/completion estimator and explicitly identify provenance through `m365.usage_source`, `usage_values_are_estimates=true`, and `usage_estimate_scope=visible_request_and_completion`. They are not Microsoft billing-token counts. The current OpenAI `stream_options.include_obfuscation` field is recognized but explicitly reported as ignored; the Sidecar does not pretend to implement stream obfuscation.
