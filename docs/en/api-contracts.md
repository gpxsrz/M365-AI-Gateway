...
When that durable witness rejects the aggregate `images` predicate, the same authenticated live record may also expose `toolCorrectionNativeEffectImageDiagnostic`. This closed, content-free diagnostic identifies only the first collected image's source class, bounded event identity, candidate-field/container/message/content-origin classes, count, and the existing classifier result after clearing only `images` on an in-memory clone. It stores neither an image URL nor a URL hash or raw field value, is excluded from JSONL and disappears on restart. It is decision telemetry only: it does not change the original transcript, guard verdict, retry count or dispatch behavior, and the durable witness remains authoritative.
