...
For `/hermes/v1/chat/completions` only, an initial rejected caller candidate can receive one model-owned syntax correction. Eligibility requires one complete, unambiguous, uniquely known and allowed tool fence, with an initial rejection classified as `illegal_escape`, `malformed_json_structure` or `unclosed_string`. Arguments must start an object; an unfinished object/string may be re-expressed by the model, but mismatched delimiters or a second value or prose after a closed object are ineligible. This boundary check neither repairs argument bytes nor proves semantic equivalence. The provider transcript must still be complete and contain only recognized text events, with no native/unknown event or artifact. Requests with `response_format`, missing/drifted conversation bindings, MCP configuration or oversized correction input are ineligible. Unknown evidence remains fail closed. This eligibility check describes the completed first response, not a provider policy that disables all native capabilities. The correction omits search and external MCP but retains the provider's existing native capability set; any observed native/unknown second result is rejected without caller acceptance. That rejection cannot undo provider actions that may already have occurred.

The classifier uses one raw transcript received by the existing collector. The following fixed contract table is evidence-bounded, not a dynamically extensible allowlist:

| JSON location / field | Type / limit | Evidence source | Operational meaning | Positive / negative |
| --- | --- | --- | --- | --- |
| type 2/3 `invocationId` | Only the existing minimal short form may omit it (type 2 keeps only its existing item/result/value shape; type 3 keeps only `type`); rich envelopes require a bounded non-empty string exactly matching outbound chat invocation `"0"` | SignalR Hub Protocol plus current `chat_payload` | Correlation, not native authority | Matching / minimal legacy; rich frames without it, number, null, empty, or wrong ID rejected |
| type 1 update `invocationId` | Optional; when present, bounded string with no equality to `"0"` | Observed M365 reverse-update shape | Passive metadata | `server-update`; wrong type rejected |
| bot-message metadata | `messageId`, `requestId`, `responseIdentifier`, `createdAt`, `timestamp`, and `turnState` are bounded strings; `turnCount` is unsigned; fields are not mutually equated | Existing M365 producer / forensic evidence | No execution authority | Correct individual types / wrong type or unknown field rejected |
| `references`, `sourceAttributions`, `adaptiveCards` | Arrays; empty arrays are supported; cards recursively accept only evidenced version `1.5` bounded `AdaptiveCard` / `TextBlock` / containers | Existing M365 metadata evidence | Cards are display-only | Empty / passive TextBlock; unknown version, non-empty references, unknown child, action, or media rejected |
| quota / `throttling` | Only evidenced `{ "remaining": unsigned integer }` | Existing collector / probe ordinary-quota shape | Counter, not success or native effect | Remaining counter; unknown key, wrong type, soft/rate limit rejected |
| `messageType=Progress` | Only the known `contentOrigin=ChainOfThoughtSummary` passive combination is metadata; its text is not an answer | Existing source-identified summary shape | Display-only | Known summary; SearchResults, Code, ToolCall, MemoryUpdate, or TriggerPlugin rejected |

Card `text` never replaces the original model text. The strict parser continues to validate the original candidate, with card and candidate retaining separate identities.

The same account, model, conversation and session receive explicit transport feedback; the rejected candidate remains memory-only. The gateway does not choose the decoded argument value, merge strings, re-upload attachments or alter canonical caller history. The replacement must be a strict JSON object for exactly the same tool, with no prose, and pass the original tool choice, count and ledger checks. Only that accepted replacement can enter the checkpoint and caller response. A malformed/drifted replacement or unsafe duplicate fails closed without a third generation or final-answer fallback. Stream and non-stream use the same gate; caller disconnect cancels a pending streamed correction. This is one extra model generation, with its outbound text included in the usage estimate; it is not a zero-cost context operation or proof of unchanged model intent.

Authenticated live diagnostics retain the initial rejection witness and separately expose `toolCorrectionAttempted`, `toolCorrectionOutcome`, `toolCorrectionFailureClass` and the original candidate hash. These existing correction fields are excluded from durable JSONL and disappear on restart. The sole exception is the closed, content-free `toolCorrectionNativeEffectWitness`: the rollback-compatible reader validates the typed schema, and the initial-response classifier atomically records it with the failure reason on the first `native_effect` rejection in the same trace record. An absent field remains compatible with existing v1 JSONL; once recorded, the bounded witness remains readable through authenticated diagnostics, survives restart and compaction, and cannot be overwritten by a corrected sibling. It means only `policy_ineligible_structure`, not that a native action executed. The witness-local `transcriptEventCount` is a bounded classifier transcript count in `1..=4096`, and every event index must be lower; top-level `eventCount=0` remains the existing redacted placeholder and does not mean that the transcript was empty. The message index is a zero-based encounter ordinal across arguments within the event; the card index is a zero-based preorder ordinal within the message. `collectorEventSha256` is SHA-256 over the complete non-empty event UTF-8 bytes after splitting a SignalR frame at the record separator (`U+001E`) and applying the existing Rust `str::trim` to outer Unicode whitespace, but before JSON parsing. It is raw collector identity, not canonical JSON: key order, Unicode escaping, number spelling and inner whitespace change the hash. No rejected candidate, feedback prompt, raw provider frame or field value is stored. Parser acceptance and the guard verdict remain unchanged.

When that durable witness rejects the aggregate `images` predicate, the same authenticated live record may also expose `toolCorrectionNativeEffectImageDiagnostic`. This closed, content-free v3 diagnostic identifies only the first collected image's source class, bounded event identity, candidate-field and closed candidate-node-type classes, container/message/content-origin classes, the exact message type only when it belongs to the fixed `allowedMessageTypes` protocol list, count, and the existing classifier result after clearing only `images` on an in-memory clone. The node-type class distinguishes absent or invalid `type`, explicit `Image` or `Media`, and an opaque `other` class without retaining the original value. `knownMessageType` is null for absent, malformed or unknown values; it never copies an unrecognized raw value. The diagnostic stores neither an image URL nor a URL hash or raw field value, is excluded from JSONL and disappears on restart. It is decision telemetry only: it does not change the original transcript, image collection, guard verdict, retry count or dispatch behavior, and the durable witness remains authoritative.
