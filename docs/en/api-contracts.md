...
Authenticated live diagnostics retain the initial rejection witness and separately expose `toolCorrectionAttempted`, `toolCorrectionOutcome`, `toolCorrectionFailureClass` and the original candidate hash. These correction fields are excluded from durable JSONL; no rejected candidate, feedback prompt or raw provider frame is stored. Existing live-store bounds apply, and restart discards the fields. Parser acceptance remains unchanged.
