...
- Hermes completion-evidence policy is bounded: a final external-action claim is allowed only when its operation matches completed typed tool evidence with an explicit successful result. Operation classification comes from the tool name or an exact terminal command verb, never an arbitrary substring in arguments; a claim naming multiple operations fails closed. Target digests use known target fields or a terminal command whose first post-verb token is a non-option target; option-only or otherwise unrecognized commands have no target binding and fail closed. A target claim is also rejected when the same claimed target maps to more than one complete target context, and multiple distinct targets remain ambiguous until the claim names the exact supported targets. Plain tool output is unknown status, unrelated read evidence does not authorize deployment, and failed or unknown matching evidence is not upgraded to success. Legacy ledgers remain readable but are degraded to unknown rather than being upgraded. Ordinary explanation, negation, history, quote, example, and explicit success/failure contradiction markers are recognized compatibility boundaries rather than a formal natural-language truth proof. The post-policy disposition/reason, duplicate tool-call suppression, and caller delivery outcome are recorded separately from the upstream transport result in non-sensitive telemetry.
