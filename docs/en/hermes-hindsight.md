...
- Enable this repository's versioned `integrations/hermes/m365_recall_provenance` plugin in Hermes. It uses only the stock `pre_llm_call` hook and `llm_request` middleware: capture the clean current-user bytes first, then create content-free HMAC metadata for the integration-appended recall range in the final request. Gateway and Hermes share a dedicated `M365_HERMES_RECALL_PROVENANCE_SECRET`; Hermes also uses `M365_HERMES_PROVIDER` to restrict the correct named provider. The secret must never appear in settings readback, logs, errors, or evidence. Missing enablement, a secret/provider mismatch, malformed ranges, message-identity drift, or text markers alone all fail closed. Hermes and Hindsight core remain unchanged.
