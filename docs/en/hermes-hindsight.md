...
- Enable this repository's versioned `integrations/hermes/m365_recall_provenance` plugin in Hermes (v1.1+). It still uses only the stock `pre_llm_call` hook and `llm_request` middleware. In addition to the existing recall/source-material range, it emits content-free HMAC provenance for Hermes' official post-tool empty-response recovery. The plugin reads Hermes' stock task-local `HERMES_SESSION_KEY` and projects that host-owned value onto the existing M365 `session_key` checkpoint seam only when there is no conflicting request value. The Gateway independently recomputes the recovery subject from that canonical `session_key` plus the normalized full transcript before accepting the HMAC, then derives the synthetic marker only in memory. The marker and recovery scaffolding are not durable checkpoint message identity. Gateway and Hermes share `M365_HERMES_RECALL_PROVENANCE_SECRET`, while `M365_HERMES_PROVIDER` restricts the named provider. The secret, raw session subject, tool result, and user text never enter provenance metadata or privacy telemetry. Caller-supplied `_empty_recovery_synthetic`, look-alike recovery text, cross-session/transcript replay, a secret/provider mismatch, hash/index drift, or an invalid signature never creates authority and fails closed. Generic `/v1` and `/memory/v1` never receive this Hermes-only synthetic authority. Hermes and Hindsight core remain unchanged.
