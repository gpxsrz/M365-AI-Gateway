...
From #75 onward, effective same-account admission is hard-capped at shared total `<=2`, Memory `<=1`, and background Hermes `<=1`, with user-originated P0 > eligible Memory P1 > background Hermes P2. `interactiveMaxConcurrent` / `memoryMaxConcurrent` remain on the settings/API compatibility surface but cannot raise those hard ceilings. `interactivePriorityHoldoffSeconds` is likewise retained as a legacy compatibility field; ordinary Memory admission no longer waits for that holdoff because priority is enforced directly by the scheduler queue policy. The Memory waiting buffer is fixed at 8 while upstream Memory concurrency remains 1.
