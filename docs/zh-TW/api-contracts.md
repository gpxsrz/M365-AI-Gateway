...
若 durable witness 拒絕的是 aggregate `images` predicate，同一筆 authenticated live record 也可能提供 `toolCorrectionNativeEffectImageDiagnostic`。這個 closed、content-free v3 diagnostic 只記第一個 collected image 的 source class、bounded event identity、candidate-field 與封閉 candidate-node-type class、container／message／content-origin class、僅限固定 `allowedMessageTypes` protocol 清單內的 exact message type、數量，以及在記憶體 clone 只清除 `images` 後交給既有 classifier 的結果。node-type class 只區分 `type` 缺省或型別錯誤、明確 `Image` 或 `Media`，以及不保存原值的 opaque `other`。`knownMessageType` 對缺省、型別錯誤或未知值一律為 null，不複製未辨識的 raw value；diagnostic 也不保存 image URL、URL hash 或其他 raw field value。它不寫 JSONL，restart 後消失，只是 decision telemetry：不修改原 transcript、image collection、guard verdict、retry count 或 dispatch behavior；durable witness 仍是 authority。
