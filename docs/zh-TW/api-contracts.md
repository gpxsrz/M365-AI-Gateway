...
若 durable witness 拒絕的是 aggregate `images` predicate，同一筆 authenticated live record 也可能提供 `toolCorrectionNativeEffectImageDiagnostic`。這個 closed、content-free diagnostic 只記第一個 collected image 的 source class、bounded event identity、candidate-field／container／message／content-origin class、數量，以及在記憶體 clone 只清除 `images` 後交給既有 classifier 的結果；不保存 image URL、URL hash 或 raw field value。它不寫 JSONL，restart 後消失，只是 decision telemetry：不修改原 transcript、guard verdict、retry count 或 dispatch behavior；durable witness 仍是 authority。
