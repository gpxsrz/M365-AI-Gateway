...
- Hermes completion-evidence policy 只做有界判定：final external-action claim 必須對應同一 operation 的 completed typed tool evidence，且結果狀態明確為成功才可放行。Operation 只能來自 tool name 或 terminal 的 exact command verb，不會掃描 arguments 任意 substring；claim 同時提到多個 operation 時 fail closed。Target digest 只取已知 target 欄位，或 terminal 中 verb 後第一個非 option token 的 exact target；只有 option、或其他無法辨識的 command 沒有 target binding，必須 fail closed。同一 claimed target 若對應多個完整 target context 也會拒絕；涉及多個 distinct target 時必須明確寫出目前支援的 exact target 才能放行。純文字 tool output 是 unknown；不相干的 read evidence 不能替 deployment 背書；matching evidence 若 failed 或 unknown 不得升級成 success。舊 ledger 仍可讀，但只降級為 unknown，不會升級成成功。普通解釋、否定、歷史、引述、範例，以及明確的成功／失敗矛盾文字只是相容性邊界，不是任意自然語言真實性的形式化證明。Post-policy disposition/reason、duplicate tool-call suppression 與 caller delivery outcome 會和 upstream transport result 分開寫入不含敏感內容的 telemetry。
