...
- Hermes 的 post-tool empty-response recovery 只有在 versioned integration 以 content-free HMAC 綁定 exact tool/result → synthetic assistant → synthetic user continuation 時，才視為同一 execution turn。Signer 從 Hermes stock task-local `HERMES_SESSION_KEY` 取得穩定 gateway session subject；M365 再用 canonical `/hermes` `session_key` 與 normalized full transcript自行重算 signed subject，因此只有 HMAC 正確仍不足以 retarget claim。完全相同 envelope 的 replay 只能重驗同一個 session/transcript subject；session、user transcript、tool call、tool result、error bit或 recovery sequence任何一項改變都會失效。同一真人 turn若連續發生多輪 authenticated recovery，只有前一輪 recovery user已先通過結構驗證時，下一輪才可延續 synthetic authority；任何未被驗證的相同 nudge文字仍是正常 user boundary。Caller 自帶 `_empty_recovery_synthetic`或偽造 metadata 都不建立 authority。Derived recovery scaffolding只屬於 execution-time state，不進 durable checkpoint message identity，所以 Hermes 在下一次正常 continuation 前移除這組 ephemeral 訊息也不會造成 checkpoint-prefix drift。
