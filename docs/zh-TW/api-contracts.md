...
若 durable witness 拒絕的是 aggregate `images` predicate，同一筆 authenticated live record 也可能提供 `toolCorrectionNativeEffectImageDiagnostic`。這個 closed、content-free v2 diagnostic 只記第一個 collected image 的 source class、bounded event identity、candidate-field 與封閉 candidate-node-type class、container／message／content-origin class、數量，以及在記憶體 clone 只清除 `images` 後交給既有 classifier 的結果。node-type class 只區分 `type` 缺省或型別錯誤、明確 `Image` 或 `Media`，以及不保存原值的 opaque `other`；不保存 image URL、URL hash 或 raw field value。它不寫 JSONL，restart 後消失，只是 decision telemetry：不修改原 transcript、image collection、guard verdict、retry count 或 dispatch behavior；durable witness 仍是 authority。
