...
- Hermes 要啟用本 repo 的 versioned `integrations/hermes/m365_recall_provenance` plugin（v1.1+）。它仍只使用 stock `pre_llm_call` hook 與 `llm_request` middleware：除了既有 recall/source-material range，也會為 Hermes 官方 post-tool empty-response recovery建立 content-free HMAC provenance。Plugin從 Hermes stock task-local `HERMES_SESSION_KEY` 取得 host-owned session subject，且只有 request沒有衝突值時才投影到既有 M365 `session_key` checkpoint seam。Gateway會用 canonical `session_key`與 normalized full transcript自行重算 recovery subject，驗證 HMAC後才在記憶體中 derive synthetic marker；marker與 recovery scaffolding都不成為 durable checkpoint message identity。Gateway與 Hermes共用 `M365_HERMES_RECALL_PROVENANCE_SECRET`，Hermes另以 `M365_HERMES_PROVIDER`限定 named provider。Secret、raw session subject、tool result與 user text都不進 provenance metadata或 privacy telemetry。Caller自稱 `_empty_recovery_synthetic`、相同 recovery文字、cross-session/transcript replay、secret/provider不符、hash/index漂移或錯誤簽章都不建立 authority並 fail closed；generic `/v1`與`/memory/v1`也不取得這項 Hermes-only synthetic authority。不修改 Hermes/Hindsight core。
